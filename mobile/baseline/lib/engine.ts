...
export class Battle {
  ...
  get objectiveText() {
    ...
    return `清除敌军 ${this.spawned - this.enemies.filter((e) => e.hp > 0).length}/${this.target}`;
  }
  createObjectives() {
  ...
  spawnEnemy(kind?: number) {
    ...
    const mult =
      ...
      (k === 3 ? 1 + Math.floor(this.mission / 6) * 0.5 : 1);
    this.enemies.push({
      id: this.nextId++,
      x,
      y,
      ...
      maxHp: data[0] * mult,
      speed:
        data[1] * (this.endless ? Math.min(1.5, 1 + this.wave * 0.025) : 1),
      damage: data[2] * [0.65, 1, 1.25][this.save.difficulty],
      rate: data[3] / [0.85, 1, 1.14][this.save.difficulty],
      radius: data[4],
      kind: k,
  ...
  shoot(t: Tank, enemy: boolean) {
    ...
    const index = enemy
      ? enemyWeapon(t.kind, Math.floor((1 - t.hp / t.maxHp) * 3))
      : this.weapon;
  ...
  step(rawDt: number, input: Input) {
    ...
    for (const e of this.enemies) {
      ...
      const difference =
        ((angle - e.turret + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
