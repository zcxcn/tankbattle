...
网页版新增「联机协作」：一人创建 2–4 人房间并分享邀请链接，队友加入后在尘湾城区合作完成歼灭突击。房主负责敌军、伤害、补给、地雷与胜负计算；WebRTC 传送队友操作和战场状态，界面显示实际选中的直连或中继路线。联机战绩不写入单人战役存档。当前公网协商入口通过 J6412 上的临时 Cloudflare Quick Tunnel 测试，隧道重启会换地址；尚无 TURN 中继，因此部分严格 NAT/运营商网络可能连不上，房间页可手动填写新的 WSS 地址。稳定公网入口与 TURN 的部署边界见 [联机设计与状态](docs/web-p2p-multiplayer-design.md)。

网页版使用固定屏幕部署菜单，战役按六章翻页，车库分为底盘、装备、成长和升级；全局关闭文字划选。进入“快速作战”，可在 **尘湾城区** 选择歼灭突击、信标防守、区域控制、车队护送、情报撤离、设施破袭、纵深突破、限时生存八种任务。有限任务均需击毁 Boss；快速作战保留坦克成长，不解锁战役章节。旧版野外地图选择会迁移到尘湾城区，成长进度保留。

敌人在北侧区域分散随机部署，每次重新开局使用新种子，并避开实体掩体、其他敌车和玩家附近。敌车在出生点附近巡逻，只有附近且视线无遮挡的目标才会引起攻击，离开巡逻范围后返回。敌军布雷每隔 35–55 秒尝试一次，普通敌车成功率 18%，Boss 为 25%；玩家布雷不变。城区保留原有街道、厂房、办公楼与货箱，建筑可逐步损坏并倒塌为可通行废墟。
