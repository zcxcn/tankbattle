...
无需安装，手机、电脑打开浏览器即可开战。支持手机双摇杆触控、键鼠和手柄。

网页版新增「联机协作」：一人创建 2–4 人房间并分享邀请链接，队友加入后由房主选择城区或山林荒野，合作完成歼灭突击。房主负责敌军、伤害、补给、地雷与胜负计算；WebRTC 传送队友操作和战场状态，界面显示实际选中的直连或中继路线。联机战绩不写入单人战役存档。当前公网协商入口通过 J6412 上的临时 Cloudflare Quick Tunnel 测试，隧道重启会换地址；尚无 TURN 中继，因此部分严格 NAT/运营商网络可能连不上，房间页可手动填写新的 WSS 地址。稳定公网入口与 TURN 的部署边界见 [联机设计与状态](docs/web-p2p-multiplayer-design.md)。

网页版使用固定屏幕部署菜单，战役按六章翻页，车库分为底盘、装备、成长和升级；全局关闭文字划选。点击“快速作战”可选择 **尘湾城区、山林荒野** 两张地图，并组合歼灭突击、信标防守、区域控制、车队护送、情报撤离、设施破袭、纵深突破、限时生存八种任务。有限任务均需击毁 Boss；快速作战保留坦克成长，不解锁战役章节。旧版已移除地图的选择会迁移到山林荒野，成长进度保留。

敌人在北侧区域分散随机部署，每次重新开局使用新种子，并避开实体掩体、其他敌车和玩家附近。敌车在出生点附近巡逻，只有附近且视线无遮挡的目标才会引起攻击，离开巡逻范围后返回。敌军布雷每隔 35–55 秒尝试一次，普通敌车成功率 18%，Boss 为 25%；玩家布雷不变。野外只有树林、圆润山丘和可以直接涉过的浅水，涉水时车速降至 72%，没有房屋或铁路。城区保留原有街道、厂房、办公楼与货箱，建筑可逐步损坏并倒塌为可通行废墟。

进入战场使用真实进度条：提前载入引擎、分批创建场景，再核验首屏材质与光照，完成首帧后开始计时和战斗；支持取消和重试。声音资源在首屏可玩后继续准备，未加载完时使用已有合成音效。较远物体和禁用废墟的着色器不再阻塞入场。

手机支持横屏与竖屏：左摇杆移动，拖动右摇杆瞄准开火，也可按住“自动瞄准”；点“布雷”布雷，点“排雷”清除范围内双方地雷。武器栏默认收起，点右侧“换武器”后按格选择，选中即收起；雷达通过右上角雷达图标开关。手机首次进入默认 30 帧、省电场景细节与高清分辨率，可在首页右上角设置中分别调整。

网页版手机新增“自适应 / 高清 / 超清”清晰度选项，默认高清按最高 2 倍 CSS 尺寸渲染，超清最高 3 倍，并分别限制在 200 万 / 320 万像素内。开启抗锯齿与更清晰的斜面贴图采样，横竖屏切换时重算像素预算；短时掉帧优先减少特效，持续负载较高时才逐步降低分辨率。设置在下一场战斗生效，存档进度保留。

战斗画布铺满整个屏幕，顶部完全透明，目标放在左上角，半透明双摇杆和技能贴近两侧。中央升级框、齐射预警横幅和顶部消息条已移除；英文字幕、升级与补给消息合并成左下角的小字，语音播报保留。Boss 血条仅在接近时出现在左侧；任务、成长详情与声音控制可在暂停菜单查看。

加农炮统一使用同一段专业录音和固定音高、低频处理，进入战斗后不因录音解码完成而中途改变音色。捡到特殊弹药只补库存，由玩家主动换武器。补给以打开的炮弹箱、维修工具箱、超频电容和装甲能源盒四类实体模型显示，共享几何缓冲和有限对象池。

网页手机适配已用浏览器模拟 320×568、360×640、390×844、430×932、568×320、844×390 尺寸检查，并试玩移动、射击、布雷、排雷和换武器；触控回归测试覆盖双指同时操作、取消触摸、暂停与旋转后的输入复位。本轮未使用实体手机验收，Android APK 未改动。
...
本轮进一步重做实际 3D 结构：车体与炮塔采用多层装甲折面，六组路轮回到履带内部，轮毂、炮口具有真正凹入的内壁；七类武器补全各自的散热、反后坐或发射机构。厂房增加连跨坡顶、锯齿采光顶和升高采光带，办公楼增加错层屋顶设备；树木改为有空隙的叶簇、分叉树干与板根，地表补充碎石和草叶。继续沿用已生成的装甲、砖墙和沥青贴图，没有新增大体积模型下载。

日光阴影现在跟随镜头，只绘制附近的投影物，车底始终保留轻量接触阴影。电影档使用更精细的阴影和抗锯齿，标准与省电档保持更低预算；结构遮蔽直接融入几何顶点，避免增加全屏后处理开销。保留三种底盘、30 级成长、七类武器和原有命中规则；仅升级网页版。
...
- 程序装甲模型新增履带绕轮、拖车耳、炮塔附加装甲、储物箱、焊缝、泥痕、炮管抽烟器、护套箍环及空心炮口。涂漆装甲采用更低金属度、较高粗糙度和结构遮蔽顶点色，保留等级进化与七种主炮。无需下载外部模型。
...
- 除基础加农炮弹药无限，六种特殊弹药初始为零。每次有效击毁保证掉落一盒弹药，拾取只增加库存，使用武器栏、数字键或 R 主动切换；一次霰弹齐射消耗一发。库存有上限，满库存保留补给，打空自动回炮，循环切换跳过空仓。Q 火箭支援也共用有限火箭库存。
...
- 加农炮使用短促炮口焰、向前扩散并上浮的烟气、贴地扬尘与快速后坐／缓慢复位的炮管动作。穿甲弹提高初速、保留原射程，沿带轻微下坠的弹道飞行，弹体朝向跟随速度；受击不会误触发炮口开火。烟气留在发射位置，暂停时冻结，手机炮口特效最多 24 片并复用网格。
