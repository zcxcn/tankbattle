...
`pc-godot/` 是用 **Godot 4.7.2、Forward+ 和 Jolt Physics** 制作的 Windows 原生坦克游戏。PC **0.4.2** 提供六关连续战役，每张地图 **288 × 384 米**：工业外围、补给港区、指挥堡垒、钢厂反击、港口封锁和最后壁垒，分别执行肃清、占领或摧毁任务，然后迎战各关首领。普通敌车逐关增加到 **6 / 9 / 12 / 15 / 18 / 22 辆**，装甲、火力和装填能力逐步增强；通关解锁、整备补给和重玩奖励均独立结算。
...
0.4.1 修复第三人称鼠标输入：接收原始屏幕位移，兼容只提供相对位移的输入，并保留细微瞄准；切换视角和暂停恢复时立即同步鼠标捕获。主炮新增更大的定向炮口焰、瞬间照明、喷气与地面扬尘，炮管在 65 毫秒内回退，再用约 0.56 秒缓慢复位，车身悬挂和镜头短促震动。关闭屏幕震动会立即清除镜头后坐，炮管机械动作仍保留。
...
坦克击毁后保留原车模型与碰撞，呈现发动机起火、炮塔环断裂或弹药舱喷火三种残骸；**25% 概率在 3.3–7.2 秒后殉爆**，火光迅速上冲形成约 10 米火柱，伴随向上喷出的碎焰、黑烟和更猛烈的炮塔抛飞，9 米内双方车辆可能受伤，建筑可以遮挡。最多保留十四辆残骸、六处持续燃烧，27 秒后熄火；暂停冻结危险计时，换关清理。

第 **2 关补给港区**加入小雨，第 **5 关港口封锁**加入较强降雨，配合阴云光照、湿润路面、局部积水、水花涟漪和原创循环雨声。降雨只在玩家周围的有限区域绘制，静态屋顶高度纹理遮挡雨滴；其他四关维持干燥天气。雨声使用音效音量，暂停时天气与声音同步停止。
...
[0.4.2 雨景、车辙与殉爆自检](docs/pc-realistic/SELF_CHECK_0.4.2.md) · [0.4.1 鼠标、炮口效果、后坐力与手柄自检](docs/pc-realistic/SELF_CHECK_0.4.1.md)
...
构建脚本固定校验官方 Godot 可执行文件和模板的 SHA-256；其他位置的同版 console 程序可通过 `-GodotPath <路径>` 指定，Windows 模板仍放在 `work/tools/godot-4.7.2/templates/`。Release 成品位于 `outputs/pc-godot/windows-x86_64/`，可分发压缩包为 `outputs/pc-godot/Iron-Embers-Windows-x86_64-0.4.2.zip`。解压后双击 `IronEmbers.exe`，保留同目录 `IronEmbers.pck` 和 `credits/` 即可离线运行。当前 Windows 可执行文件未做代码签名，首次运行可能显示系统信誉提示。
