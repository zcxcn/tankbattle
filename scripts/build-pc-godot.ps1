...
if (-not $SkipTests) {
    ...
    & $GodotPath --headless --fixed-fps 60 --path $projectRoot "res://tests/enemy_pursuit_test.tscn" -- --test
    Assert-LastExitCode -Action "Godot enemy pursuit, search memory and reacquisition"
    ...
    & $GodotPath --headless --path $projectRoot --script "res://tests/title_selection_test.gd" -- --test
    Assert-LastExitCode -Action "Godot title vehicle selection without arena rebuild"
