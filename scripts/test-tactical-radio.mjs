...
await fs.writeFile(
  path.join(output, 'tactical-radio.mjs'),
...
test('command starts once and initial Boss is announced after a clean gap', () => {
  const { b, next } = fixture();
  b.enemies = [enemy(3, 1000, 3)];
...
test('critical armor supersedes an active barrage warning after the short urgent gap', () => {
  const { b, next } = fixture();
  b.enemies = [enemy(3, 1000, 3)];
  next(0);
  next(4);
  b.boss.attackWindup = 0.8;
  assert.equal(next(4.5), 'incoming_barrage');
  b.player.hp = 10;
  assert.equal(next(4.6), null);
  assert.equal(next(4.9), 'armor_critical');
...
test('barrage decode deadline uses its remaining warning lifetime', () => {
  const { b, radio, next } = fixture();
  b.enemies = [enemy(3, 1000, 3)];
  next(0);
  next(4);
  b.boss.attackWindup = 0.5;
  b.elapsed = 4.5;
  const warning = radio.update(b);
  assert.equal(warning.id, 'incoming_barrage');
  assert.equal(warning.maxDelayMs, 500);
  assert.equal(warning.valid(), true);
  b.boss.attackWindup = 0;
  assert.equal(warning.valid(), false);
  b.boss.attackWindup = 0.3;
  b.paused = true;
  assert.equal(warning.valid(), false);
  b.paused = false;
  b.elapsed = 5.1;
...
test('barrage warning is urgent, rate limited, and never delivered after cancellation', () => {
  const { b, next } = fixture();
  b.enemies = [enemy(3, 1000, 3)];
  next(0);
  next(4);
  ...
  next(6);
  b.boss.attackWindup = 0.8;
  assert.equal(next(8), null);
  b.boss.attackWindup = 0;
  next(14);
  b.player.hp = 10;
  b.boss.attackWindup = 0.8;
  assert.equal(next(15), 'armor_critical');
  b.boss.attackWindup = 0;
  assert.equal(next(19), null);
});
