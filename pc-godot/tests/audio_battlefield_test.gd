...
func _run() -> void:
	...
	for event: String in AudioService.RADIO_CLIPS:
		...
		_check(clip.get_length() > 1.0 and clip.data.size() > 4096, "radio contains complete spoken line: " + event)
	...
	# Let the mixer consume the intentionally rapid start/stop commands before
	# tearing down the process; normal gameplay spans many rendered frames.
