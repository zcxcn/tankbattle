...
const Woodland = preload("res://data/woodland_layout.gd")
const WOODLAND_CHAPTER := 6
...
static func get_mission(index: int) -> Dictionary:
	...
	var mission := {
		"id": ["industrial_outskirts", "supply_harbor", "command_fortress", "foundry_counterattack", "harbor_blackout", "last_bastion", "woodland_valley"][chapter],
		"name": ["工业外围", "补给港区", "指挥堡垒", "钢厂反击", "港口封锁", "最后壁垒", "青岚河谷"][chapter],
		"briefing": [
			...
			"穿越丘陵树林，利用山脊遮蔽接敌。清除二十四辆巡逻战车，经三座桥渡河，摧毁北坡的苍松指挥车。土路沿缓坡延伸，林间可迂回。",
		][chapter],
		"player_start": Vector3(0, 0.05, 174),
		"boss_position": Vector3(0, 0.05, -174),
		"boss_name": ["铁牙 · 围城指挥车", "海狼 · 港区守卫", "暴君 · 堡垒指挥官", "熔炉 · 钢铁督军", "怒潮 · 封锁指挥车", "黑曜 · 最后壁垒", "苍松 · 山地指挥车"][chapter],
		...
		"enemy_count": [6, 9, 12, 15, 18, 22, 24][chapter],
		"weather": ["dry", "light_rain", "dry", "dry", "heavy_rain", "dry", "dry"][chapter],
	...
	if chapter == WOODLAND_CHAPTER:
		for key: String in ["player_start", "boss_position", "objective_position"]:
			mission[key] = Woodland.ground(mission[key])
		for i in mission.supply_positions.size():
			mission.supply_positions[i] = Woodland.ground(mission.supply_positions[i])
		for enemy in enemies:
			enemy.position = Woodland.ground(enemy.position)
			for i in enemy.patrol.size():
				enemy.patrol[i] = Woodland.ground(enemy.patrol[i])
	return mission
